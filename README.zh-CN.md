...
- **请求队列**：任务执行中继续发送链接或转发消息，会按收到顺序排队；每位用户最多等待 9999 个请求。
...
主菜单中的「🌐 语言」可选择简体中文或 English。选择保存到账号，适用于菜单、表单、确认提示、错误提示以及消息和评论提取的进度与结果。未设置时跟随 Telegram 语言：中文使用中文，其他语言使用英文；未提供语言信息时默认中文。正在运行的任务保留启动时的语言。原消息、媒体说明、文件名、自定义文字及第三方错误详情不做翻译。
...
普通停止（USER）阻止新的发送，并在传输检查点中止下载或上传。
上传时点击停止或发送 `/cancel`，原进度消息会显示确认提醒和「确认停止上传」「继续上传」按钮。
确认期间上传继续，15 秒内未确认或点击「继续上传」会恢复进度显示。
停止后重新提取需要重新下载、上传；已送达内容保留。已发出的最终发送请求仍等待返回并记录结果。
...
每条输入消息作为一个请求排队，可包含多个链接或批量范围；轮到后自动执行。
停止、失败或超时只结束当前请求，随后继续队列。排队提示可移除单个请求或清空等待队列，
清空不会停止当前任务。等待请求不占传输并发，队列保存在内存中，服务重启或账号权限撤销后清空。
排队提示在执行时直接更新为首个链接的进度与结果；进度底部显示等待请求数，无排队时显示 0。
取消和超时结果在对应任务消息中更新，已显示结果时不再追加通知。
