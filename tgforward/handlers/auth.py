...
async def handle_login_steps(client, message):
    ...
    if st.data.get("processing"):
        await dialogue.prompt(message, tr("⏳ 上一次输入仍在验证，请稍候。"))
        return
    status_msg = st.data.get("status_msg")

    if status_msg is None:
        status_msg = await message.reply(tr("⏳ 处理中..."))
    ...
    try:
        if st.step == STEP_PHONE:
            if not re.fullmatch(r"\+[1-9]\d{6,14}", text):
                await dialogue.prompt(
                    message,
                    tr("auth.phone_invalid"),
                )
                return

            await _edit(status_msg, tr("⏳ 正在发送验证码..."))
            ...
            await _edit(status_msg, tr("⏳ 正在验证验证码..."))
            try:
                code = text.replace(" ", "")
            ...
            if not await _save_login(uid, st, temp_client):
                if state.get(uid) is not st:
                    return
                await _abort_login(uid, status_msg, tr("❌ 会话保存失败，请重新 /login。"))
                return
            state.clear(uid)
            await _edit(
            ...
            temp_client = st.data["temp_client"]
            await _edit(status_msg, tr("⏳ 正在验证密码..."))
        ...
        if state.get(uid) is not st:
            return
        await _abort_login(uid, status_msg, tr("❌ 验证码无效或已过期，请重新发送 /login 再试。"))
    except BadRequest as e:
        if state.get(uid) is not st:
            return
        if st.step == STEP_PASSWORD:
            await dialogue.prompt(message, tr("❌ 密码验证失败，请重新输入密码。"))
        else:
            await _abort_login(
                uid,
                status_msg,
                tr("❌ 操作失败（{0}），请重新发送 /login 再试。", type(e).__name__),
            )
    except Exception as e:
        if state.get(uid) is not st:
            return
        logger.warning("登录流程出错 user=%s type=%s", uid, type(e).__name__)
        await _abort_login(uid, status_msg, tr("⚠️ 发生错误，请重新发送 /login 再试。"))
...
async def logout_command(client, message):
    ...
    with contextlib.suppress(Exception):
        await message.delete()
    status_msg = await message.reply(tr("⏳ 正在处理退出请求..."))
...
async def _logout_locked(uid, status_msg):
    ...
    for secret in sessions:
        ...
        try:
            ...
            if not await record_logout_pending(uid, secret, type(exc).__name__):
                await _edit(
                    status_msg, tr("❌ 远端撤销及重试记录保存失败，原凭据保留，请重新 /logout。")
                )
                return
        else:
            tracking_failed = not await clear_logout_pending(uid, secret) or tracking_failed
    ...
    if session_string and not await remove_session(uid):
        await _edit(status_msg, tr("❌ 本地会话删除失败，请重试退出登录。"))
        return
    await remove_user_client(uid)
    if remote_failed:
        await _edit(
            status_msg,
            tr("auth.revocation_pending"),
        )
    elif tracking_failed:
        await _edit(
            status_msg, tr("⚠️ 本地已退出、远端授权已撤销，但重试记录清理失败，请重新 /logout。")
        )
    else:
        await _edit(status_msg, tr("✅ 已成功退出登录，远端授权已撤销！"))
