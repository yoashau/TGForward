...
- **Request queue**: Links and forwarded messages received during a task wait in arrival order, with up to 9999 waiting requests per user.
...
A user stop (`USER`) prevents new sends and interrupts downloads or uploads at transmission checkpoints. During uploads, the stop button or `/cancel` replaces the progress display with a confirmation reminder and **Confirm stop upload** / **Continue uploading** buttons. Uploading continues; the progress display returns after 15 seconds without confirmation or when you choose **Continue uploading**. Extracting again requires downloading and uploading again; delivered content remains. A final send already issued to Telegram still returns and records its result. Timeouts (`TIMEOUT`), account revocation (`REVOKED`), and shutdown (`SHUTDOWN`) may forcibly cancel tasks. Uploading through a Premium account to the staging channel is not final delivery: the copy to the destination must succeed. FloodWait and explicit text-entity rejections permit safe retries. Comment copies explicitly rejected for access reasons may fall back to downloading and uploading.
